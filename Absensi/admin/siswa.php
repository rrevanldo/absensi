         ...
         <tr>
            ...
            <td>
               <select name="rayon" required>
                  ...
                  ?>
                        <option value="<?= $r["id_rayon"]; ?>" <?= $selected; ?> />
                           <?= $r["rayon"]; ?>
                        </option>
                  <?php 
                     }
         ...
         <tr>
            ...
            <td>
               <select name="tgl" required>
                  <?php
                     for ($tgl = 1; $tgl < 31; $tgl++) { 
                        $selected = ($tgl == $tgls) ? 'selected' : '';
                        if ($tgl <= 9) { 
                  ?>
                           <option value="<?= "0{$tgl}"; ?>" <?= $selected; ?> />
                              <?= "0{$tgl}"; ?>
                           </option>
                     <?php 
                        } else { 
               ...
               <select name="bln" required>
                  <?php
                     for ($bln = 1; $bln < 12; $bln++) {
                        $selected = ($bln == $blns) ? 'selected' : '';
                        if ($bln <= 9) { 
                  ?>
                           <option value="<?= "0{$bln}"; ?>" <?= $selected; ?> />
                              <?= "0{$bln}"; ?>
                           </option>
                     <?php
                        } else { 
               ...
               <select name="thn" required>
                  <?php
                     for ($thn = 1989; $thn < 2025; $thn++) { 
                        $selected = ($thn == $thns) ? 'selected' : '';
                  ?>
                        <option value="<?= $thn; ?>" />
                           <?= $thn; ?>
                        </option>
         ...
         <tr>
            ...
            <td>
               <a href="?menu=siswa&hapus&id=<?= $r['nis']; ?>" onclick="return confirm('Hapus Record?')">
                  <img src="../images/hapus.png" alt="Hapus" width="25" />
               </a>
            </td>
            <td>
